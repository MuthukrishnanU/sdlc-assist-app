      ...
      <div className="w-full max-w-4xl mx-auto space-y-6 relative z-10 flex-grow">
        ...
        {user.role !== 'admin' && <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            {/* Back Button Panel */}
            {/* {<div className={`p-6 rounded-3xl border glass text-center space-y-4`}>
              <div className="text-[11px] font-bold uppercase opacity-55">Navigation</div>
              <p className="text-[10px] opacity-75 leading-relaxed">
                Ready to return to the workspace? Go back to test queries on existing collections.
              </p>
              <button
                onClick={onBack}
                type="button"
                className={`w-full py-2.5 rounded-xl text-xs font-bold transition-all border flex items-center justify-center gap-1.5 hover:scale-[1.02] ${isDark
                  ? 'border-white/10 bg-white/5 hover:bg-white/10 text-white'
                  : 'border-gray-200 bg-white hover:bg-gray-50 text-gray-700 shadow-sm'
                  }`}
              >
                <ArrowLeft className="w-3.5 h-3.5" /> Return to SDLC Assist
              </button>
            </div>} */}
          </div>
        </div>}
