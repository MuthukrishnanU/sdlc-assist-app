...
async def generate_plsql(request, schema_context: str) -> dict:
    conversion_prompt = ""
    ...
    if is_cbi:
        column_projection_instruction = f"The code should query and fetch only the columns necessary to satisfy the query logic (using the provided schemas as context) out of the specified 'Columns' list: {', '.join(request.columns)}. Avoid selecting unused or redundant columns in the final output."
    else:
        column_projection_instruction = f"The code MUST query, select, and fetch all the columns specified in the 'Columns' list: {', '.join(request.columns)}, in addition to any computed or derived columns required by the logic. Do NOT omit any columns from the 'Columns' list in the final query or cursor fetch projection."

    prompt = f"""
    You are an expert database administrator and PL/SQL developer specializing in Oracle PL/SQL procedural blocks.
    {conversion_prompt}
    Your task is to generate highly optimized Oracle PL/SQL code based on the following request:
    ...
    - "generated_code": (string) The full Oracle PL/SQL code block.
    - "flow_explanation": (string) A step-by-step description of the PL/SQL cursor, variables, and procedural logic applied.
    """
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            print(f"PL/SQL generation attempt {attempt + 1} failed due to API/connection/quota error: {e}")
            raise e
        ...
        try:
            ...
            print(f"PL/SQL generation attempt {attempt + 1} failed: {e}. Retrying...")
            
    # Final fallback attempt using gpt-4o
    content, p_tok, c_tok = await call_llm(prompt + "\nNote: Generate strictly valid PL/SQL and JSON format.", "gpt-4o")
    data = parse_llm_json(content)
    data["prompt_tokens"] = p_tok
    data["completion_tokens"] = c_tok
    return data
