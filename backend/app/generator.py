    ...
    async def generate(self, request: CodeGenerationRequest) -> CodeGenerationResponse:
        try:
            ...
            3. The generated code MUST project, select, and output all the columns specified in the 'Columns' list (i.e. {", ".join(request.columns)}).
               ...
               - To prevent row duplication when joining a detail table (like `transactionsInfo`) to a customer-level table, you MUST deduplicate the final joined dataset to keep exactly one row per customer (e.g., by using PySpark `.dropDuplicates(["customer_id"])` or SQL window functions with `ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY ...) = 1`). This ensures the final output table size matches the primary entity count (e.g., 254 active home loans) while still retaining the projected transaction columns.
