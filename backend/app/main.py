...
async def push_to_github(request: GitHubPushRequest, background_tasks: BackgroundTasks):
    try:
        ...
        # 4. Trigger background task for admin email
...
@app.post("/approve-github-push/{pushId}")
async def approve_github_push(pushId: str):
    ...
    try:
        ...
        # 1. Fetch from pushAllDetailsTemp
        ...
        repo = push_request.get("repoName") or os.getenv("GITHUB_REPO")
        if not repo:
            raise HTTPException(
                status_code=400, 
        ...
        # 5. Move request from pushAllDetailsTemp to pushAllDetails
        push_log = {
...
@app.post("/reject-github-push/{pushId}")
